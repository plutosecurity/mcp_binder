...
usage() {
  cat <<'USAGE'
Usage: scripts/deploy-operator-ssh.sh --host HOST --public-ip IP --rebind-domain DOMAIN --dashboard-domain DOMAIN [options]

Packages this repo's operator VM runtime, uploads it with scp, and executes
scripts/setup-operator-vm.sh over ssh. This path is provider-neutral and works
...
run_remote_setup() {
  ...
  remote_setup="$(shell_quote "$REMOTE_DIR/setup-operator-vm.sh")"
